...
from flaskr.models.payment_type import PaymentType

bp = Blueprint('payment_types', __name__, url_prefix='/api/payment_types')
CORS(bp)
...
def create_concept():
    ...
    new_concept = PaymentType(name=data['name'])
...
def get_concept(id):
    concept = PaymentType.query.get(id)
    if concept is None:
        return {'error': 'PaymentType not found'}, 404
